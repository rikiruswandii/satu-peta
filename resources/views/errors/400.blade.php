@extends('errors::minimal')

@section('title', __('Bad Request'))
@section('content')
    <img class="nk-error-gfx" src="{{ asset('images/undraw_warning_qn4r.svg') }}" alt="">
    <div class="wide-xs mx-auto">
        <h3 class="nk-error-title">Permintaan Tidak Valid</h3>
        <p class="nk-error-text">Permintaan yang Anda buat tidak dapat diproses. Silakan periksa kembali dan coba lagi.</p>
        <a href="{{ route('/') }}" class="btn btn-lg btn-primary mt-2">Kembali ke Beranda</a>
    </div>
@endsection
